...
module.exports = class Cache extends ReinClassic {
  constructor(mysql, redis, ctx, name, options) {
    super(ctx);
    this.mysql = mysql;
    this.redis = redis;
    this.nameSpace = name;
  ...
  async load(name, args) {
    ...
    if (exists) {
      ...
      if (this.options.debug && this.options.logger) this.options.logger.info('[cache]', road, 'is from redis');
      return this.decode(existsData);
    }
    if (this.options.debug && this.options.logger) this.options.logger.info('[cache]', road, 'is from mysql');
